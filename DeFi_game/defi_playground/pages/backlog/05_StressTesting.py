...
def apply_shock_and_recover(
    ...
    if resources is not None and len(resources) > 0:
        dr["is_target"] = dr["resource"].isin([r.upper() for r in resources]).astype(int)
    else:
        dr["is_target"] = 1
    ...
    dr["volatility_shocked"] = dr["volatility"] * np.where(dr["in_shock"] == 1, cfg.vol_mult, 1.0)
    ...
    dr["slippage_shocked_raw"] = dr["avg_slippage_bps"] * (1.0 + 0.8 * (dr["volatility_shocked"] - dr["volatility"]).clip(lower=0.0))
    dr["spread_shocked_raw"] = dr["avg_spread_bps"] * (1.0 + 0.8 * (dr["volatility_shocked"] - dr["volatility"]).clip(lower=0.0))
...
resources_all = sorted(dr["resource"].astype(str).unique().tolist())
