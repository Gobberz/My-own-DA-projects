# My own DA projects

In this section I collect my own analytical projects from "life", everything that interested me in everyday life, some unique everyday questions or I want to work more with some library or technology.

## Description of projects:
| Project # | Project name (link) | What the project is about |
|---:|---|---|
| 1 | [C'était un rendez-vous](https://nbviewer.org/github/Gobberz/My-own-DA-projects/blob/main/C%27%C3%A9tait%20un%20rendez-vous/C%27%C3%A9tait%20un%20rendez-vous.ipynb) | Geospatial exploration of Claude Lelouch’s 8-minute short film *C'était un rendez-vous*: reconstructing the route and comparing how it might look today. |
| 2 | [Pigeon GPS tracker](https://nbviewer.org/github/Gobberz/My-own-DA-projects/blob/main/Pigeon_GPS/Pigeon_gps_traker.ipynb) | GPS tracking + geovisualization of a pigeon’s route in Saint Petersburg. Includes ML practice on the collected data. |
| 3 | [MoMA](https://nbviewer.org/github/Gobberz/My-own-DA-projects/blob/main/MOMA/MoMA_Comprehensive_Analysis.ipynb) | Exploratory analysis of the Museum of Modern Art (MoMA) collection dataset: patterns in artists, artworks, and acquisition trends. |
| 4 | [Online gaming behavior](https://nbviewer.org/github/Gobberz/My-own-DA-projects/blob/main/Online_gaming_behavior/Untitled.ipynb) | Analysis of user behavior in an online game: engagement, progression, and behavioral patterns. |
| 5 | [Sharks](https://nbviewer.org/github/Gobberz/My-own-DA-projects/blob/main/Sharks/Untitled.ipynb) | Analysis of global shark attack records from the 1960s to the present: trends over time, geography, and key factors. |
| 6 | [Space missions](https://github.com/Gobberz/My-own-DA-projects/blob/main/Space_missions/Space%20missions.ipynb) | End-to-end analysis of space missions since the start of the space race: launch dynamics, countries/organizations, and mission outcomes. |
| 7 | [UFO](https://nbviewer.org/github/Gobberz/My-own-DA-projects/blob/main/UFO_analytics/UFO_Analysies.ipynb) | NLP/text analysis of UFO/alien mentions in news/blog sources: topic exploration and classification of mention types. |
| 8 | [F2P-analytics](https://nbviewer.org/github/Gobberz/My-own-DA-projects/blob/main/F2P-analytics/notebooks/eda.ipynb) | A portfolio project simulating a real-world analytics workflow for a mobile free-to-play puzzle game (Homescapes-type). The project covers the full analysis cycle: synthetic data generation, statistical testing, behavioral segmentation, LTV proxy modeling, and an interactive analytics dashboard. + [Dash](https://f2p-analytics-hs.netlify.app/) |
| 9 | [DeFi_game](https://github.com/Gobberz/My-own-DA-projects/tree/main/DeFi_game) |A modular, multi-layer analytics and simulation platform for a DeFi-based game economy. Built with Streamlit, it combines real-time dashboards, tokenomics monitoring, policy simulation, fraud detection, and game-theoretic experiments into a single unified interface.|


